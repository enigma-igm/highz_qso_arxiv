...
def load_star():
    dat = ascii.read(os.path.join(root, f'resource/dwarf/combine_lris_L1.dat'))
...
def flux_to_Nlam(flux, wave_obs, sens, order):
    """
    Convert flux to Nlam
    """
    # calculate the 1/S_lambda, the factor that converts the flux to N_lambda
    S_lam_units = 1e-17 * u.erg / u.cm**2
    factor = Flam_to_Nlam(sens.wave[:,order], sens.zeropoint[:,order]).flatten() 
    facotr_interp = interpolate.interp1d(sens.wave[:,order].flatten(), factor)
    mask = (wave_obs > sens.wave[:,order][0]) & (wave_obs < sens.wave[:,order][-1])
...
def simulate(sens, spec2DObj, sobjs, header, 
             ...
             slitwidth=None, platescale=None, binning=None, sobjs_std=None,
             telluric=None, damping_wing=None, slitloss=True,
             show_trace=False, debug=False, verbose=True, show_1d=False, **kwargs):
    trace_ids = np.arange(5) * (trace_id + 1)
    all_wave = np.empty(0)
    all_counts_fake = np.empty(0)
    all_counts_real = np.empty(0)
    all_ivar_fake = np.empty(0)
    all_ivar_real = np.empty(0)
    info = {}
    for i, trace_id in enumerate(trace_ids):
        ...
        wl, Nlam = flux_to_Nlam(flux, wave, sens, order=i)
        if slitloss:
            slit_trans = simple_slit_loss(slitwidth, sobjs_fake.FWHMFIT, platescale, binning)
            slit_trans_std = simple_slit_loss(slitwidth, sobjs_std[0].FWHMFIT, platescale, binning)
            print('Median Slit transmission:', np.median(slit_trans))
            print('Median Slit transmission (std):', np.median(slit_trans_std))
            func_slit_trans = interpolate.interp1d(sobjs_fake.BOX_WAVE, slit_trans, fill_value="extrapolate")
            func_slit_trans_std = interpolate.interp1d(sobjs_std[0].BOX_WAVE, slit_trans_std, fill_value="extrapolate")
            Nlam = Nlam * func_slit_trans(wl) / func_slit_trans_std(wl)
            # Nlam = Nlam / func_slit_trans_std(wl)
        ...
        all_wave = np.append(all_wave, sobjs_fake.BOX_WAVE)
        all_counts_real = np.append(all_counts_real, sobjs[trace_id].BOX_COUNTS)
        all_counts_fake = np.append(all_counts_fake, sobjs_fake.BOX_COUNTS)
        all_ivar_real = np.append(all_ivar_real, sobjs[trace_id].BOX_COUNTS_IVAR)
        all_ivar_fake = np.append(all_ivar_fake, sobjs_fake.BOX_COUNTS_IVAR)
        ...
        if debug:
            ...
            mask_temp = (sobjs_fake_noiseless.BOX_WAVE < np.max(sobjs_fake_noiseless.BOX_WAVE)) & (sobjs_fake_noiseless.BOX_WAVE > np.min(sobjs_fake_noiseless.BOX_WAVE))
            plt.plot(sobjs_fake.BOX_WAVE[mask_temp], sobjs_fake.BOX_COUNTS[mask_temp], drawstyle='steps-mid', color='black', label='noisy')
            plt.plot(sobjs_fake_noiseless.BOX_WAVE, sobjs_fake_noiseless.BOX_COUNTS, drawstyle='steps-mid', color='blue', label='noiseless')
            plt.plot(sobjs_fake.BOX_WAVE[mask_temp], np.sqrt(inverse(sobjs_fake.BOX_COUNTS_IVAR[mask_temp])), drawstyle='steps-mid', color='red')
            plt.plot(sobjs_fake.BOX_WAVE[mask_temp], sobjs_fake.BOX_COUNTS_SIG_DET[mask_temp], drawstyle='steps-mid', color='orange')
    ...
    if show_1d:
        ...
        mask = all_wave > 5000.
        real_counts_sm, real_ivar_sm = ivarsmooth(all_counts_real[mask], all_ivar_real[mask], 5)
        fake_counts_sm, fake_ivar_sm = ivarsmooth(all_counts_fake[mask], all_ivar_fake[mask], 5)
        plt.plot(all_wave[mask], fake_counts_sm, drawstyle='steps-mid', color='black', label='fake')
        plt.plot(all_wave[mask], real_counts_sm, drawstyle='steps-mid', color='blue', label='real')
        plt.plot(all_wave[mask], np.sqrt(inverse(fake_ivar_sm)), drawstyle='steps-mid', color='red')
        plt.plot(all_wave[mask], np.sqrt(inverse(real_ivar_sm)), drawstyle='steps-mid', color='orange')
        # plt.ylim(np.median(fake_counts_sm)-3*np.median(np.sqrt(inverse(fake_ivar_sm))), 
        #         np.median(fake_counts_sm)+8*np.median(np.sqrt(inverse(fake_ivar_sm))))
        # plt.xlim(np.min(sobjs_fake.BOX_WAVE), np.max(sobjs_fake.BOX_WAVE))
        plt.title('fake vs. real object', fontsize=40)
        plt.xlabel('Wavelength [Angstrom]', fontsize=40)
        plt.ylabel('Counts', fontsize=40)
...
sim_nires_path = '../resource/simulation/nires/'
targets = ['J1007+2115']
trace_ids = [0]
redshifts = [7.515]
m_Js = [21.2581]
...
hdul = io.fits_open(os.path.join(sim_nires_path, f'{target}_tellmodel.fits'))
telluric = hdul[1].data

# the spec2d and spec1d files are from two A frames
# exposure time is 360s each, the flux level is consistent with 360s
# but the snr level is consistent with 360s*2
spec2dfile = os.path.join(sim_nires_path, f'spec2d_{target}.fits')
spec2dfile_std = os.path.join(sim_nires_path, f'spec2d_GD153.fits')
spec1dfile = spec2dfile.replace('spec2d', 'spec1d')
spec1dfile_std = spec2dfile_std.replace('spec2d', 'spec1d')

detname = DetectorContainer.get_name(det=1)
spec2DObj = spec2dobj.Spec2DObj.from_file(spec2dfile, detname, chk_version=False)
spec2DObj_std = spec2dobj.Spec2DObj.from_file(spec2dfile_std, detname, chk_version=False)
sobjs = specobjs.SpecObjs.from_fitsfile(spec1dfile, chk_version=False)
sobjs_std = specobjs.SpecObjs.from_fitsfile(spec1dfile_std, chk_version=False)
header = fits.getheader(spec1dfile)

# 1. simulate the target
# info = simulate(sens=sens, spec2DObj=spec2DObj, sobjs=sobjs, telluric=telluric, slitloss=False, 
#                 header=header, trace_id=trace_id, offset=15, exptime=720, 
#                 slitwidth=0.55, platescale=0.15, binning=1, sobjs_std=sobjs_std,
#                 load_func=load_quasar, parse_func=parse_quasar, show_trace=False, redshift=redshift, m_J=m_J, debug=False, show_1d=True)

# 2. various magnitude
# from tqdm import tqdm
# redshifts = np.arange(7.0, 8.5, 0.1)
# snr = np.zeros_like(redshifts)
# for i, redshift in enumerate(tqdm(redshifts)):
#     info = simulate(sens=sens, spec2DObj=spec2DObj, sobjs=sobjs, telluric=telluric, slitloss=False, 
#                     header=header, trace_id=trace_id, offset=15, exptime=720, 
#                     slitwidth=0.55, platescale=0.15, binning=1, sobjs_std=sobjs_std,
#                     load_func=load_quasar, parse_func=parse_quasar, show_trace=False, redshift=redshift, m_J=22, verbose=False, debug=False, show_1d=False)
#     snr[i] = info['SNR']

info = simulate(sens=sens, spec2DObj=spec2DObj, sobjs=sobjs, telluric=telluric, slitloss=False, 
                header=header, trace_id=trace_id, offset=-15, exptime=720, 
                slitwidth=0.55, platescale=0.15, binning=1, sobjs_std=sobjs_std,
                load_func=load_star, parse_func=parse_star, show_trace=False, m_J=22, verbose=False, debug=False, show_1d=False)
