...
# hdul = io.fits_open("../resource/sensfunc/Feige110_mosfire_sens.fits")
hdul = io.fits_open("../resource/sensfunc/GD153_mosfire_sens.fits")

sens = IRSensFunc.from_hdu(hdul)

sim_mosfire_path = '../resource/simulation/mosfire/'
targets = ['J1342+0928']
trace_ids = [6]
redshifts = [7.54]
m_Js = [20.6]
...
hdul = io.fits_open(os.path.join(sim_mosfire_path, f'{target}_tellmodel.fits'))
telluric = hdul[1].data

# spec2d and spec1d files are coadd of 4 exposures
# each 150s, the flux level is consistent with 300s
# the snr level is consistent with 600s

spec2dfile = os.path.join(sim_mosfire_path, f'spec2d_{target}.fits')
spec2dfile_std = os.path.join(sim_mosfire_path, f'spec2d_GD153.fits')
...
# img_fake, sobjs_fake, info = simulate(sens=sens, spec2DObj=spec2DObj, sobjs=sobjs, telluric=telluric, slitloss=False, 
#                                       header=header, trace_id=trace_id, offset=100, exptime=300, 
#                                       slitwidth=1, platescale=0.1798, binning=1, sobjs_std=sobjs_std,
#                                       load_func=load_quasar, parse_func=parse_quasar, show_trace=False, redshift=redshift, m_J=m_J, debug=True)
img_fake, sobjs_fake, info = simulate(sens=sens, spec2DObj=spec2DObj, sobjs=sobjs, telluric=telluric, slitloss=False, 
                                      header=header, trace_id=trace_id, offset=100, exptime=300, 
                                      slitwidth=1, platescale=0.1798, binning=1, sobjs_std=sobjs_std,
                                      load_func=load_quasar, parse_func=parse_quasar, show_trace=False, redshift=6.98, m_J=20.46, debug=False)

# 2. various magnitude
# redshifts = np.arange(7, 8, 0.1)
