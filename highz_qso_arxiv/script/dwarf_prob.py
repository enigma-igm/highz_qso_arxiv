...
# plt.hist(mJ_all, bins=100, histtype='step', color='k', label='All')
# plt.xlabel(r'$m_J$')
# plt.show()
...
grid_zJ = np.linspace(np.min(syn_flux[1]), np.max(syn_flux[1]), 1000000)
...
gm.fit(syn_flux_ratio)
syn_scores = gm.score_samples(syn_flux_ratio)
# get the CDF of the syn_scores
counts, bins = np.histogram(syn_scores, bins=100)
cdf = np.cumsum(counts)
cdf = cdf / cdf[-1]

dwf_scores = gm.score_samples(cand_flux_ratio[mask_dwarf])
idx = np.searchsorted(bins, dwf_scores)
dwf_prob = cdf[idx]
dwf_num = dwf_prob * 5000 * 25

plt.hist(syn_scores, bins=100, density=True, histtype='step', color='k', label='Synthetic', cumulative=True)
for i in range(len(dwf_scores)):
    plt.axvline(dwf_scores[i], color='r', alpha=0.1)
plt.xlabel(r'$\ln(p)$')
plt.ylabel('CDF')
plt.yscale('log')
plt.show()