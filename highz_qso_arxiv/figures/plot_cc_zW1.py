...
cov_zW1 = color_cov(mz_cand[mask_detected], mW1_cand[mask_detected], 
                    mJ_cand[mask_detected], mz_err_cand[mask_detected],
                    mW1_err_cand[mask_detected], mJ_err_cand[mask_detected])
cov_dwf_zW1 = color_cov(mz_cand[mask_dwarf], mW1_cand[mask_dwarf],
                        mJ_cand[mask_dwarf], mz_err_cand[mask_dwarf], 
                        mW1_err_cand[mask_dwarf], mJ_err_cand[mask_dwarf])
cov_unk_zW1 = color_cov(mz_cand[mask_unknown], mW1_cand[mask_unknown],
                        mJ_cand[mask_unknown], mz_err_cand[mask_unknown],
                        mW1_err_cand[mask_unknown], mJ_err_cand[mask_unknown])
cov_qso_zW1 = color_cov(mz_cand[mask_qso], mW1_cand[mask_qso],
                        mJ_cand[mask_qso], mz_err_cand[mask_qso],
                        mW1_err_cand[mask_qso], mJ_err_cand[mask_qso])
...
for i in range(len(redshift_bins)-1):
    qsotrack_flux_ratio[i] = np.median(qso_flux_ratio[bins_idx == i + 1], axis=0)
    qsotrack_color[i] = np.median(qso_color[bins_idx == i + 1], axis=0)
...
for i in range(len(spectype)):
    ...
    dwftrack_flux_ratio[i] = np.median(syn_flux_ratio[mask], axis=0)
    dwftrack_color[i] = np.median(syn_color[mask], axis=0)
...
ax, cnt_qso = plot_contour(qso_color[:,0],
                  -qso_color[:,4],
                  ax=ax, bins=50, range=((-2, 10), (-5, 8)),
                  levels=levels, 
                  smooth=.5,
                  ...
                  plot_datapoints=False,
                  color='red', zorder=1)
# ax, cnt_qso = plot_contour(qso_color[mask_z1][:,0],
#                   -qso_color[mask_z1][:,4],
#                   ax=ax, bins=50, range=((-2, 10), (-5, 8)),
#                   levels=levels, 
#                   smooth=.5,
...
# ax, cnt_qso = plot_contour(qso_color[mask_z2][:,0],
#                   -qso_color[mask_z2][:,4],
#                   ax=ax, bins=50, range=((-2, 10), (-5, 8)),
#                   levels=levels, 
#                   smooth=.5,
...
# ax, cnt_qso = plot_contour(qso_color[mask_z3][:,0],
#                   -qso_color[mask_z3][:,4],
#                   ax=ax, bins=50, range=((-2, 10), (-5, 8)),
#                   levels=levels, 
#                   smooth=.5,
...
qso_outer_cnt = cnt_qso.collections[0].get_paths()[0]
...
plot_cline(qsotrack_color[:,0], -qsotrack_color[:,4], ax=ax, cmap=plt.get_cmap('autumn_r'), linewidth=2, alpha=0.5, zorder=1)
plot_cline(dwftrack_color[:,0], -dwftrack_color[:,4], ax=ax, cmap=plt.get_cmap('winter_r'), linewidth=2, alpha=0.5, zorder=3)
ax.scatter(qsotrack_color[:,0], -qsotrack_color[:,4], s=20, marker='o', facecolor='red', edgecolor='red', zorder=2, alpha=0.6)
ax.scatter(dwftrack_color[:,0], -dwftrack_color[:,4], s=20, marker='o', facecolor=dwarf_contour_color, edgecolor=dwarf_contour_color, zorder=2, alpha=0.6)

ax.text(-0.8, 1.45, r'${\bf z\lesssim 6.5}$', fontsize=18, color='black', zorder=5)
ax.text(0.8, 1.45, r'${\bf z\gtrsim 6.5}$', fontsize=18, color='black', zorder=5)
ax.text(6., 1.45, r'${\bf z>7}$', fontsize=18, color='black', zorder=5)

ax.plot([-2., 0.562], [-0.261, -0.261], ls='dashed', lw=2, alpha=0.6, color='blue', zorder=2)
ax.plot([-2., 0.562], [1.239, 1.239], ls='dashed', lw=2, alpha=0.6, color='blue', zorder=2)
...
ax.plot([2.062, 9], [-0.261, -0.261], ls='dashed', lw=2, alpha=0.6, color='red', zorder=2)
ax.plot([2.062, 9], [1.239, 1.239], ls='dashed', lw=2, alpha=0.6, color='red', zorder=2)
...
mean_cov[0, 1] = -mean_cov[0, 1]
mean_cov[1, 0] = -mean_cov[1, 0]
...
if args.show:
    ...
    # plt.savefig('cc_zW1.png', dpi=200)
    plt.savefig('cc_zW1.pdf', bbox_inches='tight')